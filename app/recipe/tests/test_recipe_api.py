...
def create_recipe(user, **params):
    ...
    defaults.update(params)  # it updates the defaults with any values \
    # that were provided in **params.

    recipe = Recipe.objects.create(user=user, **defaults)
    return recipe


# make a function for making a bit easier for creating users in the system:
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_retrieve_recipes(self):
        ...
        res = self.client.get(RECIPES_URL)
        # makes the request to the recipe API
    ...
    def test_recipe_list_limited_to_user(self):
        """Test recipes for the authenticated user \
    ...
    def test_partial_update(self):
        ...
        # it should just change the title that is from payload

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        recipe.refresh_from_db()  # after some changes refresh the object in DB
    ...
    def test_update_user_returns_error(self):
        ...
        Checking that we do not update the user \
            when they call patch with a new user."""
